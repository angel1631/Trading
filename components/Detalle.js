export default ({trades, state_show})=>{
    let orden = [
        ...
        ["margen_trade","margen"],["margen_trade_usd", "margen usd"],["margen_acumulado", "margen acumulado"], ["margen_acumulado_usd", "margen acumulado usd"]
    ];
    return(
                ...
                <tbody>
            {
                trades.map((trade,index)=>{
                    ...
                    orden.map((i_orden,index)=>{
                        if(Array.isArray(i_orden)) return <td key={index}>{trade[i_orden[0]]}</td>
